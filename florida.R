...
alachua <- dat%>%
  dplyr::filter(County=='Alachua')

alachua%>%
  ggplot(aes(x = date,y=cases_new_c,colour = Age_group,label = Age_group)) +
  geom_path(na.rm = TRUE) +
  ggrepel::geom_label_repel(data = alachua%>%dplyr::filter(date==max(date))) +
  scale_y_log10() +
  facet_wrap(~County)

alachua%>%
  ggplot(aes(x=date,y=Age_group,fill=log(cases_new_c))) +
  geom_tile() +
...
dat%>%
  ...
  ggplot(aes(x=date,y=Age_group,fill=log(cases_new_c+1))) +
  geom_tile() +
  viridis::scale_fill_viridis() +
theme(
  legend.position = 'top',
  ...
  labs(
    title = '7 Day rolling Sum of New Cases by Age Group, Date',
    fill = 'New Cases (log)',
    y = 'Age group',
    x = 'Date'
  )
...
dat%>%
  ...
  labs(
    title = '7 Day rolling Sum of New Cases by Age Group, Date and County Region',
    fill = 'New Cases (log)',
    y = 'Age group',
    x = 'Date'
  )
...
dat%>%
  ...
  labs(
    title = '7 Day rolling Sum of New Cases by Age Group, Date and County',
    fill = 'New Cases (log)',
    y = 'Age group',
    x = 'Date'
  )
